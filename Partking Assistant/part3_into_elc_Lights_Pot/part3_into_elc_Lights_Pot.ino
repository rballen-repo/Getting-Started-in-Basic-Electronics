...
void loop() {
  ...
  digitalWrite(trigger, LOW);
  beepStartValue = beep_start;
  minDistValue = min_distance;
    
    //  Reading Potentiometer, which returns the distance to alert
    potDist = analogRead(pot);
    if (potDist < 1020) { 
      beepStartValue = potDist;
      }    
    minDistValue = potDist / 30;
    Serial.println(beepStartValue);
   // Serial.println(minDistValue);
